...
//    }
	...
	// get flexform and parse it
	$flexForm = $row['pi_flexform'];
	$tmp = explode("<", $flexForm);
	$index = -1;

	foreach ($tmp as $key => $val){
	    if (strpos($val, $rowFieldName) !== FALSE){
		$index = ++$key;
		break;
	    }
	}
	if ($index >= 0){
	    $spVal = explode(">", $tmp[$index]);
	    // special value from flexform
	    $spVal = intval($spVal[count($spVal) - 1]);
...
//    {
//	\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($layoutSetup);
////	die(__FILE__." : ".__FUNCTION__." -- ". __LINE__);
//
//        $specificIds = $this->helper->getSpecificIds($row);
//
...
//            }
//        }
//	\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($colCount);
//	\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($rowCount);
//        $grid .= '<colgroup>';
...
//            }
//
//	    \TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($rowConfig, $layoutRow);
//            $grid .= '<tr>';
...
//                }
//
//		\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($columnConfig, $col);
//
...
//
//
//		\TYPO3\CMS\Extbase\Utility\DebuggerUtility::var_dump($columnKey, $col);
//
//		// allowed CTypes
